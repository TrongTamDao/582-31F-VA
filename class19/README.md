...
<template id="artist-template"></template> 2. Do not forget Shadow DOM
You must actually attach a shadow root:

const shadow = this.attachShadow({ mode: "open" }); 3. Do not insert the clone into the normal DOM of the card
Append the clone into the shadow root:

shadow.appendChild(clone); 4. Do not call attachShadow() more than once
